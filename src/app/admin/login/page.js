...
export default function AdminLogin() {
  ...
  const usernameInputRef = useState('');
      ...
      <div className="w-full max-w-md glass p-8 rounded-2xl shadow-xl relative z-10">
        <div className="text-center mb-8">
          ...
          <h2 className="text-2xl font-light tracking-widest text-neutral-900 uppercase">
            ADMIN <span className="text-[#A8006E] font-semibold">PORTAL</span>
          </h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-3 px-4 rounded-lg bg-gradient-to-r from-primary to-primary-dark hover:from-primary-light hover:to-primary text-white font-semibold transition-all duration-200 transform hover:scale-[1.01] active:scale-[0.99] flex items-center justify-center gap-2 text-sm cursor-pointer glow-primary disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {loading ? (
              <>
